...
class Posts extends ActiveRecord
{
    ...
    {
        $momthes = [
            1 => 'Января', 2 => 'Февраля', 3 => 'Марат', 4 => 'Апреля',
            5 => 'Мая', 6 => 'Июня', 7 => 'Июля', 8 => 'Августа',
            9 => 'Сентября', 10 => 'Октября', 11 => 'Ноября', 12 => 'Декабря'
        ];

        $this->date = date('j',$this->date).$momthes[date('n',$this->date)].date('Y',$this->date);
    ...
    {
        $all_relases = Posts :: find()->where (['is_release' =>1])-> orderBy("date")->all();

        $number=1;

        foreach ($all_relases as $release) {
